...
(* Apply works with conditional hypotheses whose conclusions
   match the current goal _exactly_. It replaces the current
   goal with each of the hypothesis' premises as subgoals.

   Rewrite works with equality propositions where one side of the
   equality matches a _subterm_ of the current goal. It replaces
   the subterm with the other side of the equality.

   If the current goal is an equality proposition that matches
   a hypothesis exactly, either [apply] or [rewrite] can be
   used. *)
*)
(** [] *)
...
Proof.
  ...
  - intros. destruct m.
    + inversion H.
    + apply f_equal. apply IHn'. simpl in H. assumption.
Qed.
(** [] *)
